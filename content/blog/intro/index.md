...
The developer develops some packages or reuses those packages developed by others and then places them in context to each other in an AEA. The framework then calls the code in the packages. Unlike in libraries, it is the framework that runs and calls the code by making use of inversion of control](https://en.wikipedia.org/wiki/Inversion_of_control).
