...
Besides enabling Fetch.ai's vision of a DLT enabled multi-stakeholder mult-agent system I was personally also inspired by the challenge of creating a framework which allowed any user to run software under their full control, without reliance on a third party, and have that software work for them.
...
- [skills](https://docs.fetch.ai/aea/skill/): are the core focus of the framework's extensibility as they implement business logic to deliver economic value for the AEA and its owner. 
...
The developer develops some of the packages and places them in context to each other in an agent. The framework then calls the code in the packages. Unlike in libraries, frameworks make use of [inversion of control](https://en.wikipedia.org/wiki/Inversion_of_control) and are runing and calling the code.
...
Role of AI in framework?

decentralization, distribution, DLT

Developer gains
Abstracts away lots of crypto stuff > blockchain first agent framework

An AEA might act as a client or server relative to other services and clients. 

## Examples of interactions

Make protocol development easy: https://twitter.com/balajis/status/1297225036084830208
...
AEAs can also maintain servers to serve requests to other clients. For instance, an AEA might run a server to allow its owner to connect to it via a wb client.
...
3. supply off-chain data to blockchains
...
5. agent to agent interactions
	...
	AEAs shine when they are used to build multi-stakeholder agent-based solutions. Some examples we worked on include supply chain, mobility and ...

	including txs > see perun
...
- As a researchr, you can contribute to the evolution of the framework. We just painted the rough strokes, there are many gaps to fill!
...
Tools works well with: Brownie, web3.py

Reasons for many agents on one host: simulation/agent-based modelling
...
Skills where we treat complexity
